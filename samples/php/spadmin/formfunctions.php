...
function form_label($caption, $necessary=FALSE) {
  ...
  if($necessary)
    echo '<span class="red">', nl2br(htmlent_u8($caption)), '</span>';
  else
    echo htmlent_u8($caption);
  echo '</td>', "\n";
}
...
function form_caption($caption, $colspan=1) {
  if($colspan>1)
    ...
    echo '<td class="myformtd">';
  echo nl2br(htmlent_u8($caption)), '</td>', "\n";
}
...
function form_url($url, $txt, $colspan=1) {
  if($colspan>1)
    ...
    echo '<td class="myformtd">';
  echo "<a href=\"$url\">" . htmlent_u8($txt) . "</a></td>\n";
}
...
function form_list($name, $rows, $selected=-1, $default=FALSE) {
  ...
  echo '<select class="mycontrol" ',
    html_attribute("name", "form[$name]"), '>', "\n";
  echo '<option value="none">', htmlent_u8($default), '</option>', "\n";
  foreach($rows as $row) {
    echo '<option ', html_attribute("value", $row[1]);
    if($selected==$row[1])
      echo 'selected="selected" ';
    $listentry = str_replace(" ", "&nbsp;", htmlent_u8($row[0]));
...
function show_error_msg($txt) {
  echo '<p><span class="red">', htmlent_u8($txt), '</span></p>', "\n";
}
