...
function show_sp_form($mysqli, $formdata, $dbname) {
  ...
  form_new_line();
  form_label("選擇管理工作:");
  $rows = array(array('ALTER  修改現有的程序、函式', 'alter'),
                array('CALL   呼叫程序、函式', 'call'),
                array('CREATE 建立新程序', 'newp'),
                array('CREATE 建立新函式', 'newf'),
                array('DROP   刪除程序、函式', 'delete'),
                array('---', 'donothing'),
                array('備份所有 SP', 'backup'),
                array('復原 SP', 'restore'));
  form_list("action", $rows, $formdata["action"], "[選擇工作]");
  form_empty_cell();
  form_end_line();
...
function show_cmd_form($mysqli, $formdata, $dbname, $spname, $type, $action) {
  $no_of_rows = 10;  // lines in textarea for SQL cmd
  form_start("spadmin.php");
  form_new_line();
  if($action == "alter") {
    if($type=="FUNCTION") {
    ...
    form_new_line();
    form_caption("警告: MySQL 無法修改現有的程序，因此當您執行下列動作的時候，" . 
                 "會先把 $dbname.$spname 刪掉再重新建立。\n" . 
                 "如果您想建立一個與現有 SP 相似的新 SP，就不要使用這個功能，因為您會失去原有的 SP！");
    form_end_line();
  } elseif($action == "newp") {
    ...
    form_new_line();
    form_asis("<b>這是您想呼叫的 SP 程式碼內容:</b>" .
              "<br /><br />" .
              str_replace(" ", "&nbsp;", nl2br(htmlspecialchars(substr($code, 7)))) .
              "<br /><br />您可以用分號隔開數個 SQL 指令。");
    form_end_line();
    $no_of_rows = 5;
  }
...
function test_sp($mysqli, $dbname, $cmd) {
  echo "<h3>執行 SQL 指令的結果</h3>\n";
  $mysqli->select_db($dbname);
  printsql($cmd);
  $ok = $mysqli->multi_query($cmd);
  if($mysqli->info)
    printerror("資訊: " . $mysqli->info);
  if($mysqli->warning_count)
    printerror("警告: " . $mysqli->warning_count);
  if($mysqli->errno)
    printerror("錯誤: " . $mysqli->error);
...
function show_table($result) {
  if($result->num_rows>0 && $result->field_count>0) {
    ...
    foreach($result->fetch_fields() as $meta)
      printf("<th class=\"myth\">%s</th>", htmlent_u8($meta->name));
    echo "</tr>\n";
    ...
    while($row = $result->fetch_row()) {
      ...
      foreach($row as $col)
        printf("<td class=\"mytd\">%s</td>",
          str_replace(" ", "&nbsp;", nl2br(htmlent_u8($col))));
      echo "</tr>\n";
    }
    echo "</table></p>\n";
  }
...
function printsql($sql) {
  printf("<p><font color=\"#0000ff\" size=\"-1\">%s</font></p>\n",
         str_replace(" ", "&nbsp;", nl2br(htmlent_u8($sql))));    }
...
function build_href($url, $query, $txt) {
  if($query)
    return "<a href=\"$url?" . $query . "\">" . htmlent_u8($txt) . "</a>";
  else
    return "<a href=\"$url\">" . htmlent_u8($txt) . "</a>";
}
