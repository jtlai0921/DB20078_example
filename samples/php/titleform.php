...
function build_form($formdata) {
  ...
  // explain
  form_new_line();
  form_asis("您至少必須指定標題與一位作者。" .
    "<br />作者: 先寫姓再寫名 " .
    "(<i>King Stephen</i>, 不是 <i>Stephen King</i>)！" .
    "<br />如果書籍擁有許多位作者的話，可以用分號將他們分開" .
    " (<i>author1;author2</i>)。" .
    "<br />出版社、語言: 可以在現有資料內選擇，" .
    "也可以輸入新的資料。", 4);
  form_end_line();
  ...
  form_url("categories.php", "編輯類別", 2);
  form_end_line();
  ...
  form_new_line();
  form_label("");
  form_button("btnSave", "儲存");
  // delete button for existing titles
  if(array_item($formdata, "titleID")) {
    form_button("btnDelete", "刪除書籍");
    form_hidden("titleID", $formdata["titleID"]); }
  else
    form_empty_cell(1);
  form_button("btnClear", "輸入新書資料 (清除表單)");
  form_end_line();
...
function build_delete_form($formdata) {
  ...
  if($titleID = array_item($formdata, "titleID")) {
    ...
    if($rows = $db->queryObjectArray($sql)) {
      form_start("titleform.php");
      form_new_line();
      form_caption("真的要刪除 " . $rows[0]->title . " 這本書嗎？", 2);
      form_end_line();

      form_new_line();
      form_button("btnReallyDelete", "是的，刪除這本書");
      form_button("btnClear", "不，取消");
      form_end_line();
